...
// vérification de l'unicité de email :
  $email_verif = $mysqli->query("SELECT * FROM radio WHERE email = '$_POST[email]'");
...
// insertion en base :
    $mysqli->query("INSERT INTO radio (nom, nom_res, prenom_res, adresse, ville, cdp, pays, email, tel, statut, inscription, login, password)

     VALUES ('$_POST[nom]','$_POST[nom_res]','$_POST[prenom_res]', '$_POST[adresse]', '$_POST[ville]','$_POST[cdp]','$_POST[pays]', '$_POST[email]','$_POST[tel]',0,NOW(),'$_POST[login]','$_POST[password]')") or die($mysqli->error);
...
echo $msg;
 ...
 <br />
  ...
  <div class="tab-pane active" id="contact_details">
    ...
    <!-- \\formulaire d' inscription onglet contact -->

      <div class="form-row">
         <div class="form-group col-md-5">
         <!-- <label>Enter Nom du Responsable </label> -->
         <input type="text" name="nom_res" id="nom_res" class="form-control" style="text-transform: uppercase;" placeholder="Nom du responsable"/>
         <span id="error_nom_res" class="text-danger"></span>
        </div>

        <div class="form-group col-md-5">
         <!-- <label>Enter Prénom du Responsable </label> -->
         <input type="text" name="prenom_res" id="prenom_res" class="form-control" style="text-transform: uppercase;" placeholder="Prénom du responsable"/>
         <span id="error_prenom_res" class="text-danger"></span>
        </div>
      </div>
         
        <div class="form-group col-md-4">
         <!--<label>Enter Nom de la Radiologie </label> -->
         <input type="text" name="nom" id="nom" class="form-control"  placeholder="Nom de la Radiologie "/>
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-3">
         ...
         <input type="text" name="tel" id="tel" class="form-control" style="text-transform: capitalize;" placeholder="Telephone"/>
     ...
     <div class="form-group col-md-10">
      ...
      <!-- \\formulaire d'inscrpition onglet inscription -->
  ...
  <div class="tab-pane fade" id="login_details">
      ...
      <div class="form-row">
        ...
        <!-- \\checkbox show hide -->
         ...
         <!-- \\checkbox cvg -->
         <div><input type="checkbox" value="check" required="required"n disabled="disabled" checked="checked" name="acceptation" id="acceptation">  J'ai lu et j'accepte les <A href="cvg/cvg.php"> conditions d'utilsation</A></div>
      </div>  
...
$(document).ready(function(){
  ...
  {
   error_nom = 'Nom de la radio requis ';
  ...
  else
  {
   ...
   $('#nom').removeClass('has-error');
    ...
    if($.trim($('#nom_res').val()).length == 0)
  {
   error_nom_res = 'Nom du responsable requis';
   $('#error_nom_res').text(error_nom_res);
   $('#nom_res').addClass('has-error');
  }
  else
  {
   error_nom_res = '';
   $('#error_nom_res').text(error_nom_res);
   $('#nom_res').removeClass('has-error');
  }

  if($.trim($('#prenom_res').val()).length == 0)
  {
   error_prenom_res= 'Prénom du responsable est requis';
   $('#error_prenom_res').text(error_prenom_res);
   $('#prenom_res').addClass('has-error');
  }
  else
  {
   error_prenom_res= '';
   $('#error_prenom_res').text(error_prenom_res);
   $('#prenom_res').removeClass('has-error');
  }
  ...
  if(error_nom != '' ||  error_nom_res!= '' || error_prenom_res != '' || error_adresse != '' || error_cdp != '' || error_ville != '' || error_pays != '' ||error_tel != '' || error_email != '') 
  {
   return false;
  }
  else
